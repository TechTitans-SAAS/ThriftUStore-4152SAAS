...
When('I fill in the comment form with {string}') do |comment_text|
  within('.form-group') do
    # Wait for the Trix editor to be present
    find('trix-editor', visible: :all)
    # Fill in the Trix editor input field
    find('input[name="comment[body]"]', visible: :all).set(comment_text)
  end
end

When('I edit the comment form with {string}') do |new_comment_text|
  within('.edit-comment-form') do
    #find('.trix-editor', visible: :all)
    #find('input[name="comment[body]"]', visible: :all).set(new_comment_text)
    find('trix-editor').click.set(new_comment_text)
  end
end
